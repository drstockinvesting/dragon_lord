...
`index.html` currently loads **nothing**. No `<img>`, no sprite sheet, no audio file, no
font file, no `fetch`, no network request of any kind. Every dragon is a stroked vector
path built at startup; every sound is synthesised from oscillators and filtered noise.
...
### 1. Boot becomes asynchronous — this is the real one
...
1. **Background / parallax layer.** One image, one draw call, no alpha problem, no
   animation frames, no hitbox coupling. Biggest visual change for the least code.
